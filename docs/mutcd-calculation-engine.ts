...
export function calculatePedestrianAccommodations(
  ...
  const needsPassingSpaces = workZoneLengthFeet > 200;
...
export function determineWarningLights(
  ...
  const isStationary = duration !== 'mobile';
...
export function calculateWorkZoneRequirements(
  ...
  const laneWidth = input.customLaneWidthFeet || 12;
  const offsetWidth = laneWidth * input.closedLanes.length;
...
export function generateEquipmentPlacement(
  ...
  const internalWidth = workSpaceLengthFeet;
...
export function checkConfigurationWarnings(input: WorkZoneInput): string[] {
  ...
  if (input.hasBicycleFacility && !bicycle) {
    warnings.push('No bicycle accommodation specified - verify if facility can remain open');
  }
